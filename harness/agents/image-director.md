...
4. 카드뉴스 시리즈 (cadence: series-3 / series-5):
   - 첫 카드 = HOOK (오버사이즈 헤드라인, 시선 잡기)
   - 중간 카드 = BODY (한 슬라이드에 통계 1개 또는 짧은 단락 1개)
   - 마지막 카드 = CTA (마무리, 브랜드 컬러 강하게)
   - generate.mjs 가 cadence → cardCount 자동 매핑하고 카드별 role hint 를 prompt 에 주입
