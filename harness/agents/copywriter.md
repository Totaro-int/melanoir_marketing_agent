...
## Goal별 추천 단어 카테고리 (사전이 없을 때 fallback)

| campaign goal | 자주 쓰는 카피 단어 카테고리 |
|--------------|--------------------------|
| `launch` | 드디어, 신제품, 인생을_바꾼, 첫_공개 |
| `awareness` | 문제, 실수, 갭_만들기, 의외성 |
| `conversion` | 단(저렴), 한정, 고객만족도, 검증된 |
| `engagement` | 자, 자_이제, 함께, 우리 |
| `education` | 입문, 알아두면, 정리, 가이드 |

이 카테고리에 해당하는 단어가 spec에 없더라도 카피 작성 시 그 nuance를 의식적으로 활용한다.
