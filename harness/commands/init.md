...
3. **`/doctor`** — 빨간 항목이 없는지 확인
4. **`/onboard`** — 회사 프로필 인터뷰
5. **`/campaign new "<주제>"`** — 첫 캠페인
6. **`/generate <slug>`** → **`/preview <slug>`** → **`/approve`**
7. **`/auth add <channel>`** + **`/publish ... --dry-run`** → 페이로드 확인 후 실 발행
...
- [ ] dry-run 페이로드 사람 검토
- [ ] LinkedIn / Threads OAuth 토큰 발급 → `/auth add`
- [ ] 실 발행 1회 (single 카드)
- [ ] series-3 카드뉴스 1회
- [ ] `/status --watch` 로 보드 확인
