...
| 호출 | 동작 |
|------|------|
| `/queue tick` | 한 번 실행. 도래한 항목 처리. |
| `/queue tick --dry-run` | 시뮬레이션 (실제 발행 안 함). |
| `/queue install [--every=15]` | 자동 실행 설치 (macOS launchd 또는 crontab). 기본 15분 간격. |
| `/queue uninstall` | 자동 실행 제거. |
| `/queue status` | 자동 실행 설치 여부 확인. |
...
`needs_attention` 으로 표시된 항목은 사람이 `/preview` 후 직접 수정해야 함.
