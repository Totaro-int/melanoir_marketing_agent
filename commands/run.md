...
`/run "<주제>" --channels=threads,linkedin [--cadence=single|series-3|series-5] [--approve] [--publish] [--dry-run]`

내부 동작:
1. `company-profile.yaml` 존재 확인 (없으면 사용자에게 `/onboard` 안내)
...
`/run schedule "<주제>" --channels=threads --period=week|month --frequency=N [--time=09:00] [--start=YYYY-MM-DD] [--titles="t1|t2|t3"] [--no-auto-publish] [--no-generate]`
...
3. 자동 실패 (가드 reject, 인증 만료 등) → `status=needs_attention` + `attentionReason[ch]` 기록 → 사람이 `/preview` 후 손으로 수정
4. `--no-auto-publish` 주면 알림만 (사람이 `/publish` 직접 호출)
...
사용자가 `/run` 만 입력하고 인자가 없으면 다음을 차례로 묻기:
