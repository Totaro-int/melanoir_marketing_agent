...
실패 항목이 있으면 detail 컬럼에 다음 액션을 명시 (예: `npm install`, `cp .env.example .env.local`, `/onboard`). exit code 0/1 로 CI 게이트에도 사용 가능.

## /init 도 비슷한 일을 하나요?

`/init`(skill)은 **새 사용자 첫 가이드** — `bin/setup.mjs` 를 권장하고 `/onboard` 로 넘김. `/doctor`는 **언제든 환경 점검**.
